  ...
  if (window.confirm("Are you sure you want to delete this member?")) {
    try {
      ...
      if (res.data.success) {
        ...
        Allmember();  // ✅ Correct function to refresh members
      } else {
  ...
  return (
    <div>
      ...
      <div className="max-w-7xl mx-auto p-4 bg-white rounded-2xl shadow-lg mt-8 mb-10">
        <h2 className="text-xl lg:text-2xl font-bold mb-6 text-gray-700 text-center">
          Delete Gym Members
        </h2>
              ...
              {member && member.length > 0 ? (
                member.map((m, index) => (
                  <tr key={index} className="border-b hover:bg-orange-50">
                    ...
                    <td className="p-3 text-gray-600 tracking-wide">
                      {m.phone}
                    </td>
                    ...
                    <td className="p-3 text-xs text-gray-500">
                      {m.membership?.endDate
                        ? new Date(m.membership.endDate).toLocaleDateString()
                        : "N/A"}
                    </td>
                    <td className="p-3">
                      <button
